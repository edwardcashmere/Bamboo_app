defmodule BambooApp.SubscriptionTest do
  ...
  describe "user_category_subscriptions" do
    ...
    test "get_user_subscribers_by_category/1 returns the user_category_subscription with given id" do
      ...
      assert [%UserCategorySubscription{user_id: ^user_1_id}, %UserCategorySubscription{user_id: ^user_2_id}] =
    ...
    test "delete_user_category_subscription/1 deletes the user_category_subscription", %{user_subscription: user_subscription} do

      assert {:ok, %UserCategorySubscription{}} =
               Subscription.delete_user_category_subscription(user_subscription)

      assert [] =
        Subscription.get_user_subscribers_by_category(user_subscription.category_id)
    end

    test "change_user_category_subscription/1 returns a user_category_subscription changeset", %{user_subscription: user_subscription} do

      assert %Ecto.Changeset{} =
               Subscription.change_user_category_subscription(user_subscription)
    end
  end
end
